...
CMD sh -xc "cd /data; [ ! -f '/data/genesis.json' ] && unzip /$NETWORK'net.zip' && geth --datadir.ancient . --datadir ./datadir init genesis.json && sed -i '/^HTTP/d' ./config.toml; exec geth --config ./config.toml --datadir.ancient . --datadir ./datadir --pprof --pprof.addr 0.0.0.0 --metrics —-http -—http.api eth,net,web3,txpool,parlia --http.corsdomain '*' --http.vhosts '*' --http.addr 0.0.0.0 --http.port 8545 --ws --ws.api eth,net,web3 --ws.origins '*' --ws.addr 0.0.0.0 --ws.port 8546 --graphql --graphql.addr 0.0.0.0 --graphql.port 8587 --graphql.corsdomain '*' --graphql.vhosts '*'"
